...
g = InteractionMatrixFromPhaseScreen(ngs=src,
                                      atm=atm,
                                      tel=tel,
                                      wfs=pywfs,
                                      ...
                                      stroke=param['stroke'])
#%%
gF = g.D[:,:param['nModes']] # modes2slopes for reconstructed modes
gR = g.D[:,param['nModes']:] # modes2slopes for higher order modes
gFplus = np.linalg.pinv(np.transpose(gF) @ gF) @ np.transpose(gF)

crosstalkMatrix = gFplus @ gR
...
# allow memory for saving telemetry
...
outputPywfs = iterative_estimator(param['diameter'],
                              ...
                              m=param['nModesShown'],# crosstalkMatrix.shape[0]*crosstalkMatrix.shape[1], 
                              c_mat=crosstalkMatrix,
...
data['crosstalkMatrix'] = crosstalkMatrix
data['gF'] = gF
data['gR'] = gR
data['gFplus'] = gFplus
